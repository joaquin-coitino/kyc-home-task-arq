...
- Alternative explanation: data quality issues
...
- The two datasets are inconsistent for this user or could be a manual override
...
- Original `decision_label=APPROVED` — this seems a deliberate manual override
...
- May indicate a different workflow (e.g. NFC, digital identity) where usability is not required for the document credential or a data error — needs clarification
...
- `OK` (8 rows) and `APPROVED` (4 rows) in decision_label — `APPROVED` might mean manual approval but unclear
